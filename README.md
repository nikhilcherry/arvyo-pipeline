...
Note: `tests/fixtures/` (generated by `scripts/generate_fixtures.py`) use a
~27-day baseline (one TESS sector) at 10-minute cadence, dense/long enough
for a blind BLS/TLS search to actually recover the injected periods (the
planet fixture's 3.14-day period shows 8+ transits). `null`/`unknown`
correctly fail the SDE/SNR gate (`no_period`).
