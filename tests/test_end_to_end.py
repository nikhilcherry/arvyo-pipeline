...
`scripts/regenerate_fixtures.py`) use a 27.4-day baseline (one TESS sector)
at 30-minute cadence — dense/long enough for a blind BLS/TLS search to
actually recover the injected 3.14-day planet period (8 full transits) and
the 1.5-day EB period. See `tests/test_fixture_recovery.py` for the direct,
non-glue-layer regression on that claim. `null`/`unknown` correctly fail the SDE/SNR gate
